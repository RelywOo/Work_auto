...
def integration_env(tmp_path):
    ...
    mock_tg.download_topic_content = AsyncMock(
        return_value={"photos": 3, "text_messages": 5}
    )
...
async def test_full_pipeline_happy_path(integration_env):
    ...
    equipment_json = json.dumps(
        {"demontaj": ["DCDU12B", "Anten T1003"], "montaj": ["RRU 5516"]}
    )
    photo_results = [
        json.dumps(
            {"is_demontaj": True, "equipment_found": "DCDU12B", "reason": "Старое"}
        ),
        json.dumps(
            {"is_demontaj": True, "equipment_found": "Anten T1003", "reason": "Ржавое"}
        ),
        json.dumps(
            {"is_demontaj": False, "equipment_found": "RRU 5516", "reason": "Новое"}
        ),
    ]
    call_count = {"n": 0}
    ...
    with (
        patch("modules.ai_processor.genai") as mock_genai,
        ...
        with patch(
            "modules.ai_processor.AIProcessor._generate_with_retry",
...
async def test_pipeline_empty_topic(integration_env):
    ...
    env["mock_tg"].download_topic_content = AsyncMock(
        return_value={"photos": 0, "text_messages": 0}
    )
...
async def test_graceful_shutdown_waits_for_worker(integration_env):
    ...
    assert processing_finished.is_set(), (
        "Worker task should have completed before shutdown returned"
    )
    tq._process_topic_heavy.assert_called_once()
...
async def test_shutdown_force_cancels_after_timeout(integration_env):
    ...
    assert worker_task.done(), (
        "Worker task should be done after shutdown (force-cancelled)"
    )
