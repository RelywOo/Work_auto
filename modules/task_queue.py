...
class TaskQueue:
    ...
    async def add_topic_task(
        self, topic_id: int, topic_title: str, priority: int = 1
    ) -> None:
        ...
        self.logger.info(
            f"📋 Topic {topic_title} (ID: {topic_id}) added to processing queue"
        )
        ...
        while not self._shutting_down.is_set():
            try:
                ...
                unprocessed_files = await loop.run_in_executor(
                    self.executor,
                    self.telegram_client.db.get_unprocessed_files,
                    topic_id,
                )
                ...
                if last_msg_id is None:
                    ...
                    self.logger.info(
                        f"🤖 Topic {topic_id} has {len(unprocessed_files)} unprocessed AI files"
                    )
                    should_process = True
                else:
                ...
                self.logger.info(
                    f"🔨 {worker_name} starting topic {task_data['topic_title']} (ID: {topic_id})"
                )

                # Process in a separate thread to avoid blocking the event loop
                try:
                    await loop.run_in_executor(
                        self.executor, self._process_topic_heavy, task_data
                    )
                    ...
                    self.logger.info(
                        f"✅ {worker_name} finished topic {topic_id} in {processing_time:.1f}s"
                    )
                    ...
                    self.logger.error(
                        f"❌ {worker_name} failed to process topic {topic_id}: {e}"
                    )
    ...
    def _download_phase(
        ...
        future = asyncio.run_coroutine_threadsafe(
            self.telegram_client.download_topic_content(
                topic_id, photo_message_limit=PHOTO_MESSAGE_LIMIT
            ),
            self.main_loop,
        )
        ...
        self.logger.info(
            f"DEBUG [worker thread]: unprocessed files after download = {len(unprocessed_files)}, topic_id = {topic_id}"
        )
        return download_result, unprocessed_files

    def _ai_analysis_phase(
        self,
        topic_id: int,
        topic_title: str,
        is_vdo: bool,
        site_id: str,
        unprocessed_files: List[Tuple[int, str]],
        topic_dir: str,
        log_file_path: str,
    ) -> Tuple[int, int, int, Dict[str, List[str]], bool, bool, bool]:
        ...
        try:
            ...
            if unprocessed_files:
                self.logger.info(
                    f"🤖 Found {len(unprocessed_files)} files for AI processing"
                )
                ...
                if is_vdo:
                    self.logger.info(
                        f"🔍 VDO topic: searching main topic for site {site_id}..."
                    )
                    try:
                        ...
                        vdo_main_topic_id = main_topic_future.result(
                            timeout=VDO_SEARCH_TIMEOUT
                        )
                        if vdo_main_topic_id:
                            self.logger.info(
                                f"Main topic found (ID: {vdo_main_topic_id}). Downloading text logs..."
                            )
                            text_log_future = asyncio.run_coroutine_threadsafe(
                                self.telegram_client.get_topic_text_log(
                                    vdo_main_topic_id
                                ),
                                self.main_loop,
                            )
                            ...
                            if text_log:
                                ...
                                with open(main_log_path, "w", encoding="utf-8") as f:
                                    f.write(text_log)
                                self.logger.info(
                                    "Extracting equipment lists from main topic..."
                                )
                                ...
                                has_demontaj = bool(equipment_lists.get("demontaj"))
                                self.logger.info(
                                    f"✅ Merged equipment lists: demontaj={len(equipment_lists['demontaj'])}, montaj={len(equipment_lists['montaj'])}"
                                )
                        else:
                            self.logger.warning(
                                f"⚠️ Main topic not found for site {site_id}"
                            )
                    except Exception as e:
                        self.logger.error(
                            f"❌ Error searching/downloading main topic: {e}"
                        )
                ...
                if not is_vdo:
                    ...
                    try:
                        ...
                        vdo_twin_topic_id = vdo_twin_future.result(
                            timeout=VDO_SEARCH_TIMEOUT
                        )
                        if vdo_twin_topic_id:
                            has_vdo_twin = True
                            self.logger.info(
                                f"🔗 VDO twin found (ID: {vdo_twin_topic_id}). Downloading text logs..."
                            )
                            text_log_future = asyncio.run_coroutine_threadsafe(
                                self.telegram_client.get_topic_text_log(
                                    vdo_twin_topic_id
                            ...
                            if text_log:
                                ...
                                with open(vdo_log_path, "w", encoding="utf-8") as f:
                                    f.write(text_log)
                                self.logger.info(
                                    "Extracting equipment lists from VDO twin..."
                                )
                                ...
                                has_demontaj = bool(equipment_lists.get("demontaj"))
                                self.logger.info(
                                    f"✅ Merged equipment lists from VDO twin: demontaj={len(equipment_lists['demontaj'])}, montaj={len(equipment_lists['montaj'])}"
                                )
                        else:
                            self.logger.info(f"ℹ️ No VDO twin found for site {site_id}")
                    except Exception as e:
                        self.logger.error(
                            f"❌ Error searching/downloading VDO twin: {e}"
                        )
                ...
                if os.path.exists(log_file_path) and os.path.getsize(log_file_path) > 0:
                    if is_vdo or has_vdo_twin:
                        ...
                        if not has_montaj and not has_demontaj:
                            ...
                            self.logger.warning(
                                f"⚠️ Equipment lists empty for VDO-path topic '{topic_title}', continuing"
                            )
                        else:
                            self.logger.info(
                                "✅ Equipment lists found, continuing processing"
                            )
                    else:
                    ...
                    if is_vdo or has_vdo_twin:
                        ...
                        missing_text_log = True
                        self.logger.info(
                            "ℹ️ VDO-path topic without text report. Continuing."
                        )
                        if is_vdo:
                            equipment_lists["demontaj"] = [
                                "Любое старое/демонтированное оборудование VDO"
                            ]
                    else:
                ...
                if is_vdo and vdo_main_topic_id:
                    try:
                        ...
                        self.logger.info(
                            f"📸 Downloaded {main_photos} montaj photos from main topic"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"❌ Error downloading photos from main topic: {e}"
                        )
                ...
                if not is_vdo and has_vdo_twin and vdo_twin_topic_id:
                    try:
                        ...
                        self.logger.info(
                            f"📸 Downloaded {vdo_photos} demontaj photos from VDO twin"
                        )
                    except Exception as e:
                        self.logger.error(
                            f"❌ Error downloading photos from VDO twin: {e}"
                        )

                # === Stage A: Classify all photos (without moving files) ===
                self.logger.info(f"{'=' * 60}")
                self.logger.info(
                    f"📸 STAGE 1: Classifying {len(unprocessed_files)} photos..."
                )
                self.logger.info(f"{'=' * 60}")
                classification_results = []  # [(message_id, file_path, ai_result)]
                total_files = len(unprocessed_files)

                for file_num, (message_id, file_path) in enumerate(
                    unprocessed_files, 1
                ):
                    try:
                        if is_vdo:
                            ai_result = "demontaj"
                            self.logger.info(
                                f"[{file_num}/{total_files}] 🚀 {os.path.basename(file_path)} → VDO → DEMONTAJ"
                            )
                        elif has_vdo_twin:
                            ai_result = "montaj"
                            self.logger.info(
                                f"[{file_num}/{total_files}] 🔗 {os.path.basename(file_path)} → VDO twin → MONTAJ"
                            )
                        else:
                        ...
                        classification_results.append(
                            (message_id, file_path, ai_result)
                        )

                    except Exception as e:
                        self.logger.error(
                            f"[{file_num}/{total_files}] ❌ Error classifying {os.path.basename(file_path)}: {e}"
                        )
                ...
                if not is_vdo and not has_vdo_twin and len(classification_results) >= 3:
                    ...
                    for idx in outlier_indices:
                        ...
                        context = []
                        for j in range(
                            max(0, idx - 2), min(len(classification_results), idx + 3)
                        ):
                            if j == idx:
                                context.append("?")
                            else:
                        ...
                        try:
                            ...
                            new_result = (
                                "demontaj" if reanalysis["is_demontaj"] else "montaj"
                            )
                            ...
                            self.logger.error(
                                f"❌ Error re-analyzing outlier {os.path.basename(fpath)}: {e}"
                            )

                # === Stage C: Move files and update DB ===
                self.logger.info(f"{'=' * 60}")
                self.logger.info(
                    f"📁 STAGE 3: Moving {len(classification_results)} files to folders..."
                )
                self.logger.info(f"{'=' * 60}")
                for message_id, file_path, ai_result in classification_results:
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"❌ Error processing file {filename}: {move_error}"
                        )

                demontaj_count = (
                    len(os.listdir(demontaj_dir)) if os.path.exists(demontaj_dir) else 0
                )
                montaj_count = (
                    len(os.listdir(montaj_dir)) if os.path.exists(montaj_dir) else 0
                )
                unknown_count = 0

                self.logger.info(
                    f"📊 File organization: Demontaj={demontaj_count}, Montaj={montaj_count}, Unknown={unknown_count}"
                )
            ...
            self.logger.error(
                f"🚨 Critical AI processing error for topic {topic_id}: {e}"
            )
    ...
    def _packaging_phase(
        ...
        topic_dir: str,
        log_file_path: str,
        equipment_lists: Dict[str, List[str]],
    ) -> str:
    ...
    def _send_and_cleanup_phase(
        ...
        zip_path: str,
        topic_dir: str,
        has_vdo_twin: bool = False,
        missing_text_log: bool = False,
    ) -> None:
        ...
        try:
            ...
            if demontaj_count > 0 or montaj_count > 0 or unknown_count > 0:
                ...
                recog_pct = (
                    int(((demontaj_count + montaj_count) / total) * 100)
                    if total > 0
                    else 0
                )
            ...
            send_timeout = SEND_FILE_TIMEOUT_BASE + int(
                file_size_mb * SEND_FILE_TIMEOUT_PER_MB
            )
            self.logger.info(
                f"📤 Sending ZIP archive to Telegram ({zip_path}, {file_size_mb:.1f} MB, timeout={send_timeout}s)..."
            )
            asyncio.run_coroutine_threadsafe(
                self.telegram_client.send_file_with_retry(
                    "me", file=zip_path, caption=summary
                ),
                self.main_loop,
        ...
        try:
            ...
            self.logger.error(
                f"⚠️ Error during cleanup for {topic_dir}: {cleanup_error}"
            )
    ...
    def _process_topic_heavy(self, task_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if (
                download_result["photos"] > 0
                or download_result["text_messages"] > 0
                or unprocessed_files
            ):
                ...
                if should_stop:
                    try:
                        self.logger.info(
                            f"🧹 Cleaning up folder on early stop: {topic_dir}"
                        )
                        safe_rmtree(topic_dir)
                        ...
                        for msg_id, file_path in unprocessed_files:
                            if hasattr(self.telegram_client, "processed_msg_ids"):
                                if msg_id in self.telegram_client.processed_msg_ids:
                                    self.telegram_client.processed_msg_ids.remove(
                                        msg_id
                                    )

                        self.logger.info(
                            f"💾 DB and cache cleared: bot forgot topic {topic_id}"
                        )
                    except Exception as cleanup_error:
                        self.logger.error(
                            f"⚠️ Error during cleanup/DB reset for {topic_dir}: {cleanup_error}"
                        )
                    return
                ...
                if os.path.exists(existing_zip):
                    self.logger.info(
                        f"♻️ Found existing ZIP from previous run: {existing_zip}"
                    )
                    zip_path = existing_zip
                else:
                    zip_path = self._packaging_phase(
                        topic_id,
                        topic_title,
                        site_id,
                        topic_dir,
    ...
    def get_metrics_summary(self) -> str:
        ...
        avg_time = (
            (m["total_processing_time"] / m["topics_processed"])
            if m["topics_processed"] > 0
            else 0.0
        )
        return (
