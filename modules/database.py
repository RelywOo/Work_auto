...
class DatabaseManager:
    ...
    def save_message(self, message_id: int, topic_id: int, message_type: str, file_path: Optional[str] = None) -> None:
    ...
    def clear_topic_state(self, topic_id: int) -> None:
        ...
        with self._get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM processed_messages WHERE topic_id = ?", (topic_id,))
            cursor.execute("DELETE FROM processed_topics WHERE topic_id = ?", (topic_id,))
            conn.commit()
