...
class AIProcessor:
    ...
    async def extract_equipment_lists(self, log_file_path: str) -> Dict[str, List[str]]:
        ...
        try:
            ...
            ВАЖНО — сохраняй ВСЮ информацию из лога для каждой позиции:
            - Количество (шт.) — если указано число перед или после названия, добавь его через " — N шт"
            - Метраж кабеля — если указана длина (метры), добавь через " — Xм"
            - Сечение/тип кабеля — сохраняй как часть названия (например "2x10", "3x2.5")

            Пример ответа: {"demontaj": ["Anten T1003M6R011 — 2 шт", "Кабель силовой 2x10 — 85м", "DCDU12B"], "montaj": ["RRU 5516 — 3 шт"]}.
            Если количество или метраж не указаны — не добавляй, пиши только название.
